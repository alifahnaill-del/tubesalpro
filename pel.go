...
// ==================== SELECTION SORT RATING MENTOR ====================

// Selection Sort untuk mengurutkan mentor berdasarkan Rating (descending)
func sortMentorByRatingDesc(mentor []Mentor) []Mentor {
	if len(mentor) <= 1 {
		return mentor
	}
	
	// Buat salinan slice agar tidak mengubah data asli
	sorted := make([]Mentor, len(mentor))
	copy(sorted, mentor)
	...
	for i := 0; i < len(sorted)-1; i++ {
		...
		for j := i + 1; j < len(sorted); j++ {
			if sorted[j].Rating > sorted[maxIdx].Rating {
				maxIdx = j
			}
		}
		// Tukar jika ditemukan rating yang lebih tinggi
		if maxIdx != i {
...
// Fungsi untuk menampilkan mentor yang sudah di-sort berdasarkan rating
func menuSortMentorByRating() {
	cetakHeader("MENTOR TERBAIK (RATING TERTINGGI)")
	
	if len(daftarMentor) == 0 {
		fmt.Println(ColorYellow + "  Belum ada data mentor." + ColorReset)
		return
	}
	
	// Sort mentor berdasarkan rating
	sortedMentor := sortMentorByRatingDesc(daftarMentor)
	
	// Tampilkan dengan format yang lebih menarik
	fmt.Printf("\n  %s%-6s %-4s %-28s %-22s %-7s %-5s %-8s %-12s%s\n",
		ColorBold+ColorCyan, "Rank", "ID", "Nama Mentor", "Bidang", "Rating", "Exp", "Siswa", "Biaya", ColorReset)
	cetakGaris(100, "─")
	
	for i, m := range sortedMentor {
		...
		if i == 0 {
			fmt.Printf("  %s%-6s %-4d %-28s %-22s %-7.1f %-5d %-8d %-12s%s\n",
				ColorYellow, rank, m.ID, m.Nama, bidang, m.Rating, m.Pengalaman, m.JumlahSiswa, formatRupiah(m.Biaya), ColorReset)
		} else if i == 1 {
			fmt.Printf("  %s%-6s %-4d %-28s %-22s %-7.1f %-5d %-8d %-12s%s\n",
				ColorCyan, rank, m.ID, m.Nama, bidang, m.Rating, m.Pengalaman, m.JumlahSiswa, formatRupiah(m.Biaya), ColorReset)
		} else if i == 2 {
			fmt.Printf("  %s%-6s %-4d %-28s %-22s %-7.1f %-5d %-8d %-12s%s\n",
				ColorGreen, rank, m.ID, m.Nama, bidang, m.Rating, m.Pengalaman, m.JumlahSiswa, formatRupiah(m.Biaya), ColorReset)
		} else {
			fmt.Printf("  %s%-6s %-4d %-28s %-22s %-7.1f %-5d %-8d %-12s%s\n",
				ColorReset, rank, m.ID, m.Nama, bidang, m.Rating, m.Pengalaman, m.JumlahSiswa, formatRupiah(m.Biaya), ColorReset)
		}
	}
	cetakGaris(100, "─")
	fmt.Printf("\n  %sTotal Mentor: %d | Rating tertinggi: %.1f | Rating terendah: %.1f%s\n", 
		ColorCyan, len(sortedMentor), sortedMentor[0].Rating, sortedMentor[len(sortedMentor)-1].Rating, ColorReset)
}
...
func tampilkanMenuTampilkanData() {
	...
	menus := []string{
		...
		" 4. Mentor Terbaik (Rating Tertinggi)",
		" 0. Kembali ke Menu Utama",
	}
	for _, m := range menus {
		fmt.Printf("  %s\n", m)
